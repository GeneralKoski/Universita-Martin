...
extern double* block_failed_ct;  /// count how many leaves are found for each block level
extern int block_size;
extern double* block_thr;  /// for each block, threshold (number of allowed leaves for each block)
extern int n_blocks;
extern int* block_failed;  /// flag to detect the failure of a block

int flag_first_sol_found=1;

void search_best_heur_bbf(int lev, int coming_from_lev)
{
  ...
  int curr_block=(lev-1)/block_size;  // block number
  int prev_block=(coming_from_lev-1)/block_size;  // block number
  ...
  if (lev==1) 
  {
     // if ssbonds are in use, let find the first conformation, before 
     // using the bbf strategy
     if (nssbond>0) 
       flag_first_sol_found=0;
     else
       flag_first_sol_found=1;     
  }
  
  if (SEARCH_DBG) printf("Level %d, prev %d\n",lev,coming_from_lev);
  ...
  if (lev>n)
  {
           ...
           if (best_energy>temp) 
           {
             ...
             printf("Best new minimum: %f, leaves%d\n",best_energy,leaves);
//             for (int i=0;i<n;i++)
...
{     for (int i=0;i<n_blocks;i++)
     ...
     printf("\n");
     for (int i=0;i<n_blocks;i++)
     printf("%3.0f ",block_failed_ct[i]);
     printf("\n");
     for (int i=0;i<n_blocks;i++)
     printf("%3.0f ",block_thr[i]);
     printf("\n");
}     
//           for (int i=0;i<n_blocks;i++)
//            block_failed_ct[i]=0;
   ...
   if (curr_block!=prev_block) /// start of a new block
   {
          block_failed_ct[curr_block]=0;
          if (SEARCH_BBF_DBG) printf("Start block %d [%d..%d]\n",curr_block,curr_block*block_size,(curr_block+1)*block_size-1);

          if (SEARCH_BBF_DBG)
          {
     printf("************** SETTING NEW BL ******************\n");
     printf("Lev %d block %d, in bl%d\n",lev,curr_block,(lev-1)%block_size);
     for (int i=0;i<n_blocks;i++)
     printf("%3d ",i);
     printf("\n");
     for (int i=0;i<n_blocks;i++)
  ...
  if (block_failed_ct[curr_block]>=block_thr[curr_block])
     // if when returning jump to another block, inc the prev block
     flag_return=1;        
...
//// assign every point in the domain and open branches
       go_on=1;
       while(go_on && 
             block_failed_ct[curr_block]<block_thr[curr_block])  
       {
...
//               printf("\t Lev%d: FAST%d: Trying value %d %d %d for var%d\n",lev,fast_dir_ct,fast_choice1,fast_choice2,fast_choice3,variable);
	     ...
	     if (go_on)
	     {
           ...
           ////// try a fast pattern indomain
                      ...
                      for (int i=beg;i<=end && flag1;i++)
                      {
                        ...
                        if (STATE->variables[i].l[0]>p1 ||
                            ...
                            STATE->variables[i].h[2]<p3)
                              flag1=0;
                    if (SEARCH_DBG) printf("point %d, sh%d val %d %d %d pattern %d %d %d\n",i,pt_sh,p1,p2,p3,pattern[pattern_code[variable]][h][pt_sh][0],
                    pattern[pattern_code[variable]][h][pt_sh][1],pattern[pattern_code[variable]][h][pt_sh][2]);
	        ...
	        real_positions[i][2]=sh[2]+pattern_real[pattern_code[variable]][h][pt_sh][2];
               ...
               //            printf("\n");           
                          search_best_heur_bbf(lev+n_var,lev);         /// proceed in tree exploration
                      }
                ...
                // == NORMAL ============================================================================                    
                  search_best_heur_bbf(lev+1,lev);
             } 
	       }	       
	     }/// go_on
     ...
     if (SEARCH_DBG) printf("Leave lev %d, going to %d\n",lev, coming_from_lev);
     if (curr_block!=prev_block &&
         prev_block>=0)
         {
          if (flag_first_sol_found) 
          block_failed_ct[prev_block]++;
          if (SEARCH_BBF_DBG)
if (curr_block<5)
          {
     printf("[********* FAIL BBF * inc bl %d *********************\n",prev_block);
     printf("Lev %d block %d, in bl%d\n",lev,curr_block,(lev-1)%block_size);
     for (int i=0;i<n_blocks;i++)
     printf("%3d ",i);
     printf("\n");
...
void search_best_heur_bbf(int lev)
{
  search_best_heur_bbf(lev,0);
}
