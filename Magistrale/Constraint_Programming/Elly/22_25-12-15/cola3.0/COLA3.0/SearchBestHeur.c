...
//////////// propagation         
	     if (go_on)
	     {
           ...
           ////// try a fast sse indomain
                int beg,end;
                int ct=0;   
                if (code_helix[variable]>0)
                {  
                // =================================================================================                    
                // == HELIX ========================================================================                    
                  beg=helix[code_helix[variable]-1][0];
                  end=helix[code_helix[variable]-1][1];     
                  ...
                  if (ct==1) 
                  {
                    ...
                    if (SEARCH_DBG) printf("Beg: %d end %d, Pos%d\n",beg,end,variable);
                    int n_var=end-beg+1;     
                    int ofs=(variable-beg)%4; /// pattern offset
                    /// for each helix placement, try to assign, test consistency and propagate
                    for (int h=0;h<24;h++)
                    {
                      /// assign domain       
                    ...
                    if (SEARCH_DBG) printf("Helix %d; SH%d %d %d\n",h,sh[0],sh[1],sh[2]);
                      int pt_sh;   /// compute pattern shift
                      int pt_ofs;  /// and offset in pattern
       
                      pt_sh=(variable-beg)/4;   /// compute pattern shift
                      pt_ofs=(variable-beg)%4;  /// and offset in pattern
                      //printf("sh%d of%d\n",pt_sh,pt_ofs);
                      /// subtract the helix absolute position (correct traslation to overlap helix in point "var")
                      sh[0]-=pattern_h[h][pt_ofs][0]+pt_sh*pattern_h_dir[h][0];
                      sh[1]-=pattern_h[h][pt_ofs][1]+pt_sh*pattern_h_dir[h][1];
                      sh[2]-=pattern_h[h][pt_ofs][2]+pt_sh*pattern_h_dir[h][2];
         
                      int p1,p2,p3;
                      int flag1=1;
                      for (int i=beg;i<=end && flag1;i++)
                      {
                        pt_sh=(i-beg)/4;   /// compute pattern shift
                        pt_ofs=(i-beg)%4;  /// and offset in pattern
                        //if (SEARCH_DBG) printf("i:%d var%d: pt%d sh%d\n",i,variable,pt_sh,pt_ofs);
                        p1=sh[0]+pattern_h[h][pt_ofs][0]+pt_sh*pattern_h_dir[h][0];
                        p2=sh[1]+pattern_h[h][pt_ofs][1]+pt_sh*pattern_h_dir[h][1];
                        p3=sh[2]+pattern_h[h][pt_ofs][2]+pt_sh*pattern_h_dir[h][2];                        
                        if (STATE->variables[i].l[0]>p1 ||
                            ...
                            STATE->variables[i].h[2]<p3)
                              flag1=0;
                    if (SEARCH_DBG) printf("point %d, sh%d of%d val %d %d %d\n",i,pt_sh,pt_ofs,p1,p2,p3);
                        /// and assign the variable
                        var_init_point(STATE->variables+i,p1,p2,p3);
                        /// set the flag for labeling
                        STATE->variables[i].labelled=1;
                        /// note that for current variable, the contacts are already computed!!
                    ...
                    //if (flag1) printf("Cons\n");
                      ...
                      if (flag1 && cstore_ac(STATE,trailtop)) /// arc consistency on non ground vars
                          search_best_heur(lev+n_var);         /// proceed in tree exploration
                ...
                // =================================================================================                    
                // == STRAND =======================================================================                    
                if (code_strand[variable]>0)
                {
                  beg=strand[code_strand[variable]-1][0];
                  end=strand[code_strand[variable]-1][1];
                  for (int i=beg;i<=end;i++)
                    if (STATE->variables[i].labelled==1) ct++;
                  if (ct==1)
                  {
                    select_type_indomain=2;  
//                    printf("Fast strand possible\n");                            
                   //printf("Beg: %d end %d, Pos%d\n",beg,end,var);
                   int n_var=end-beg+1;     
                   int ofs=(variable-beg)%2; /// pattern offset     
                   /// for each strand placement, try to assign, test consistency and propagate
                     /// store current state
                     allvar_fill_from(temp_nodes+lev,STATE);
                   for (int h=0;h<24;h++)
                   {     
                   ...
                   //printf("strand %d; SH%d %d %d\n",h,sh[0],sh[1],sh[2]);
                     int pt_sh;   /// compute pattern shift
                     int pt_ofs;  /// and offset in pattern       
                     pt_sh=(variable-beg)/2;   /// compute pattern shift
                     pt_ofs=(variable-beg)%2;  /// and offset in pattern
                     //printf("sh%d of%d\n",pt_sh,pt_ofs);
                     /// subtract the helix absolute position (correct traslation to overlap helix in point "var")
                     sh[0]-=pattern_s[h][pt_ofs][0]+pt_sh*pattern_s_dir[h][0];
                     sh[1]-=pattern_s[h][pt_ofs][1]+pt_sh*pattern_s_dir[h][1];
                     sh[2]-=pattern_s[h][pt_ofs][2]+pt_sh*pattern_s_dir[h][2];         
                     int p1,p2,p3;
                     int flag1=1;
                     for (int i=beg;i<=end && flag1;i++)
                     {           
                       pt_sh=(i-beg)/2;   /// compute pattern shift
                       pt_ofs=(i-beg)%2;  /// and offset in pattern
                       //printf("i:%d var%d: pt%d sh%d\n",i,var,pt_sh,pt_ofs);         
                       p1=sh[0]+pattern_s[h][pt_ofs][0]+pt_sh*pattern_s_dir[h][0];
                       p2=sh[1]+pattern_s[h][pt_ofs][1]+pt_sh*pattern_s_dir[h][1];
                       p3=sh[2]+pattern_s[h][pt_ofs][2]+pt_sh*pattern_s_dir[h][2];
                       if (STATE->variables[i].l[0]>p1 ||
                           STATE->variables[i].l[1]>p2 ||
                           STATE->variables[i].l[2]>p3 ||
                           STATE->variables[i].h[0]<p1 ||
                           ...
                           STATE->variables[i].h[2]<p3)
                           flag1=0;                          
                       //printf("point %d, sh%d of%d val %d %d %d\n",i,pt_sh,pt_ofs,p1,p2,p3);
                       /// and assign the variable
                       var_init_point(STATE->variables+i,p1,p2,p3);
                       /// set the flag for labeling
                       STATE->variables[i].labelled=1;
                     ...
                     if (flag1 && cstore_ac(STATE,trailtop)) /// arc consistency on non ground vars
                          search_best_heur(lev+n_var);         /// proceed in tree exploration
                     /// At the end of current strand try, restore the original state
                      allvar_fill_from(STATE,temp_nodes+lev);
                   }/// for end   
                  }
                }
