# 00 - Calendario di studio verso l'orale

> 🛑 **CALENDARIO SCADUTO - le date qui sotto non valgono più.** L'orale del 10 settembre 2026 **non si sostiene** (deciso il 07/09/2026): anche questo piano, come quello dell'11 agosto, non è mai partito. Nessuno dei blocchi 25 agosto - 9 settembre è stato svolto.
>
> **Quello che resta valido è la sequenza**, non il calendario: l'ordine in cui aprire i materiali, il tempo stimato per blocco, gli avvisi ⚠️ sui punti in cui si inciampa e le note su cosa il prof può chiedere. Leggilo come una scaletta, ignorando i giorni della settimana.
>
> **Nuovo bersaglio: appello della sessione invernale (gennaio-febbraio 2027)**, data da verificare su Esse3. Le date vanno riancorate a quell'appello **quando la data esce e quando ci sono ore di studio reali alle spalle** - non si scrive un terzo calendario in anticipo, visto che i primi due sono stati scritti e non eseguiti. Contesto e prossime azioni in [`../TODO.md`](../TODO.md).
>
> ⚠️ **Prima di studiare, va accertata una cosa:** il seminario del 23 giugno 2026 (30% del voto) vale "per tutti gli appelli dell'**a.a. 2025/26**", e settembre 2026 era probabilmente l'ultimo. Se va rifatto per l'a.a. 2026/27, l'argomento va riconcordato col docente **almeno 30 giorni prima**: è la prima cosa da chiedere nella mail, non l'ultima. Bozza in [`07_mail_prof.md`](07_mail_prof.md).

Questo file è la scaletta operativa. I blocchi tematici e il metodo stanno in [`README.md`](README.md); qui c'è *in che ordine apro le cose*.

---

## Il bilancio del piano fallito: 16 giorni, ~35h, zero margine
...
Il piano precedente aveva 30 giorni per 32h: si poteva sforare senza conseguenze; questo no, con ~5h di margine. Nei fatti è sforato tutto, perché non è iniziato: **la capienza di 2h nei giorni lavorativi era un'ipotesi, non un dato osservato.** Da rifare la stima sulla sessione invernale partendo dalle ore che riesci effettivamente a mettere in una settimana normale di lavoro a Dieffetech.

Il carico residuo resta quindi **~35h piene**: sotto, la ripartizione per blocco è ancora quella da smaltire.

### Cosa era stato tagliato per far entrare tutto (tagli da riconfermare)

- **Laboratorio: da 4h a 1.5h.** Sopravvive solo la parte interrogabile all'orale, cioè `calc-2` vs `calc-2-prec` con `calc.output` sotto gli occhi (blocco 5B). I 4 binari di TinyP, il `diff` LLVM `-O0`/`-O2` e la modifica al lexer Flex diventano **opzionali**: si fanno solo se un blocco chiude in anticipo.
- **Domande tipo: un passaggio completo + uno selettivo**, non due completi (blocchi 13 e 14).
- **Domande incrocio: 2h invece di 3**, compresse nei blocchi 14-15 insieme alla simulazione.

Non è stato toccato: il parsing (4h piene + 1.5h di Bison, è l'argomento coperto peggio) e il ripasso finale (blocchi 14-16).

Con una sessione invernale più lontana questi tagli possono anche essere annullati: il laboratorio pieno (TinyP, `diff` LLVM `-O0`/`-O2`, lexer Flex modificato) e il secondo giro completo di domande sono il primo posto in cui rimettere ore, se ci sono.

Per il laboratorio del blocco 5B serve **bison**, che sulla macchina Windows non è installato (flex e g++ sì).
...
## Fase 1 - tutto il front end (~16h)

### 1. Inquadramento + avvio lexer (2h)
...
### 2. Analisi lessicale completa (2h)
...
### 4. Parsing II, tabella LL(1) sulle mani (2h)
...
### 5. Parsing III bottom-up + Bison (4h)
...
### 6. Analisi dipendente da contesto + IR (4h)
...
## Fase 2 - back end, middle end, memorizzazione (~16h)

### 7. Astrazione procedurale (2h)
...
### 9. Ottimizzazioni locali e dominatori (2h)
...
### 11. Interpretazione astratta (2h)
...
### 13. 62 domande a voce, passaggio completo (4h)
...
## Fase 3 - finale (~6h, gli ultimi giorni prima dell'appello)

### 14. Recupero esitazioni + incrocio (2h)
...
### 16. Ripasso intensivo (la sera prima, 2h)
...
- **I tre blocchi finali (14-16) sono intoccabili.** Se qualcosa sfora, si sacrificano nell'ordine: il laboratorio opzionale (TinyP, `diff` LLVM), poi le domande incrocio D2/D3, poi il secondo passaggio sulle domande. Mai il ripasso, mai il parsing
- **Il recupero è nel weekend, non "domani".** È la regola che non è stata rispettata ad agosto: un blocco rimandato che scivola in avanti si porta dietro tutti gli altri, e due piani di fila sono morti così
