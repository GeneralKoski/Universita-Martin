...
// Definisco una struttura per tenere traccia dei nodi visitati e dei loro livelli
struct NodeInfo {
    int level;
    bool visited;
};
...
// Funzione DFS ricorsiva per visitare il grafo e calcolare la lunghezza del ciclo più lungo
int DFS(int n, NodeInfo* nodeInfo, int parent, int& longestCycle) {
    // Marca il nodo corrente come visitato
    nodeInfo[n].visited = true;

    // Inizializza la lunghezza del ciclo da questo nodo a 0
    int cycleLengthFromThisNode = 0;

    // Espande la lista di adiacenza del nodo corrente
    node_t *elem = E[n]->head;
    while (elem != NULL) {
        int neighbor = elem->val;

        // Se il vicino non è stato visitato, visita ricorsivamente il vicino
        if (!nodeInfo[neighbor].visited) {
            nodeInfo[neighbor].level = nodeInfo[n].level + 1; // Incrementa il livello del vicino
            int cycleLength = DFS(neighbor, nodeInfo, n, longestCycle);
            longestCycle = max(longestCycle, cycleLength);
        } 
        // Se il vicino è già stato visitato e non è il genitore del nodo corrente, calcola la lunghezza del ciclo
        else if (neighbor != parent) {
            longestCycle = max(longestCycle, nodeInfo[n].level - nodeInfo[neighbor].level + 1);
        }

        elem = elem->next;
    }

    return cycleLengthFromThisNode;
}

void visitDisconnectedGraph() {
    // Inizializziamo l'array per tenere traccia dei nodi visitati e dei loro livelli
    NodeInfo* nodeInfo = new NodeInfo[n_nodi];
    for (int i = 0; i < n_nodi; ++i) {
        nodeInfo[i].level = -1; // -1 indica che il nodo non è stato visitato
        nodeInfo[i].visited = false;
    }

    // Per ogni nodo, se non è stato visitato, avvia una nuova DFS
    for (int i = 0; i < n_nodi; ++i) {
        if (!nodeInfo[i].visited) {
            // Per il primo nodo, impostiamo il suo livello a 0
            nodeInfo[i].level = 0;
            int longestCycle = 0;
            DFS(i, nodeInfo, -1, longestCycle);
        }
    }
...
int main(int argc, char **argv) {
	...
	// Inizializziamo la struttura NodeInfo
	NodeInfo* nodeInfo = new NodeInfo[n_nodi];
    for (int i = 0; i < n_nodi; ++i) {
        nodeInfo[i].level = -1; // -1 indica che il nodo non è stato visitato
        nodeInfo[i].visited = false;
	...
	// Visita il grafo incluso il caso di grafi disconnessi
    visitDisconnectedGraph();

    // Calcola la lunghezza del ciclo più lungo
    int longestCycle = 0;
    DFS(0, nodeInfo, -1, longestCycle); // Partendo da qualsiasi nodo

    // Output della lunghezza del ciclo più lungo
    cout << "La lunghezza del ciclo piu' lungo nel grafo e': " << longestCycle << endl;
	...
	delete[] nodeInfo; // Liberiamo la memoria allocata per nodeInfo
    return 0;
}
