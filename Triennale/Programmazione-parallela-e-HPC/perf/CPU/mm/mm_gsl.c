...
if (gsl)  // GSL
 ...
 {
  gsl_matrix_view A = gsl_matrix_view_array(a, NRA, NCA);
  gsl_matrix_view B = gsl_matrix_view_array(b, NCA, NCB);
  gsl_matrix_view C = gsl_matrix_view_array(c, NRA, NCB);

  gsl_blas_dgemm( CblasTrans, CblasNoTrans,  1.0, &A.matrix, &B.matrix, 0.0, &C.matrix);
 }
else
 {
   for(i=0;i<NRA;i++)
...
printf("# N  gflop  tempo(s)  GFLOP/s  Hostname  \n");
...
{
  int i;
   while ( (i = getopt(argc, argv, "n:vgh")) != -1) {
        switch (i)
        {
