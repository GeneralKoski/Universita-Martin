...
./heat_gpu -h
./heat_gpu -r 4096 -c 4096 -x 32 -y 32 # x e y sono la dimensione del blocco 2D
*/
...
int block_size_x=16;  // --- Block dim
int block_size_y=16;  // --- Block dim
...
int main(int argc, char **argv)
{
  ...
  fprintf (stderr,"# NX=%d, NY=%d,  block_size_x=%d, block_size_y=%d, grid_size_x=%d, grid_size_y=%d, MAX_ITER=%d \n", \\
               NX, NY, block_size_x, block_size_y, iDivUp(NX, block_size_x), iDivUp(NY, block_size_y), MAX_ITER);
...
///////////////////////////////
   ...
   wtime = (double) ( t2.tv_sec - t1.tv_sec )
    ...
    fprintf(stderr,"GPU, %d, %d, %d, %d, %d, %d, %d,  %f\n",\\
       NX, NY, block_size_x, block_size_y,  iDivUp(NX, block_size_x), iDivUp(NY, block_size_y),  MAX_ITER, wtime);
...
void options(int argc, char * argv[]) {
  ...
  int i;
   while ( (i = getopt(argc, argv, "x:y:c:r:s:h")) != -1) {
        switch (i) {
        case 'x':  block_size_x = strtol(optarg, NULL, 10);  break;
        case 'y':  block_size_y = strtol(optarg, NULL, 10);  break;
        case 'c':  NX       = strtol(optarg, NULL, 10);  break;
        case 'r':  NY       = strtol(optarg, NULL, 10);  break;
        case 's':  MAX_ITER = strtol(optarg, NULL, 10);  break;
        case 'h':  printf ("\n%s [-x block_size_x] [-y block_size_y] [-c ncols] [-r nrows] [-s nsteps] [-h]\n",argv[0]); exit(1);
        default:   printf ("\n%s [-x block_size_x] [-y block_size_y] [-c ncols] [-r nrows] [-s nsteps] [-h]\n",argv[0]); exit(1);
        }
    }
}
