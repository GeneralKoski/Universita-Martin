...
int main(int argc, char **argv)
{
  ...
  double time, Tstart, Tstop, Tj, Ttot;
...
//    copy_cols(h_T_old, NX, NY);
    ...
    Tj += (double) ( stop.tv_sec - start.tv_sec )
          + (double) ( stop.tv_nsec - start.tv_nsec ) / BILLION;
    
    if (debug) { system("clear"); print_colormap(h_T_old); sleep(1);  }
  ...
  fprintf(stderr,"# num_threads, NX, NY, MAX_ITER, time_totale, time_jacobi\n");
  fprintf (stderr,"OMP, %d, %d, %d, %d, %.3f, %.3f\n", omp_get_max_threads(), NX, NY, MAX_ITER, Ttot, Tj);
