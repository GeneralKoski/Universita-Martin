...
int main(int argc, char **argv)
{
  ...
  struct timespec start, stop, t_start, t_stop;
  double time, t_tot;
  ...
  t_tot = (double)(t_stop.tv_sec - t_start.tv_sec) + (double)(t_stop.tv_nsec - t_start.tv_nsec) / BILLION;

  fprintf (stderr,"%d, %d, %d, %.3f, %.3f \n", NX, NY, MAX_ITER, time, t_tot);
