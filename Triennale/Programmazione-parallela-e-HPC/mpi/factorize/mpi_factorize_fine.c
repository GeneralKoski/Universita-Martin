...
// Domain decomposition
    ...
    if (block_addr_start)
        block_addr_size = block_addr_start + 1;
...
// Attiva la Ricezione della comunicazione dagli altri rank
    ...
    for (block_idx=block_addr_size-MPIrank-1; block_idx >= 0 ; block_idx= block_idx-MPIsize){
...
{
    int i;
    while ( (i = getopt(argc, argv, "a:b:m:s:h")) != -1) {
        switch (i)
        {
